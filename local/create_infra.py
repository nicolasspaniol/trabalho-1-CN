...
def ensure_security_group(ec2, name, description):
...
def get_or_create_target_group(elbv2):
    try:
...
def get_or_create_load_balancer(elbv2, alb_sg_id):
    try:
...
def setup_worker_infrastructure(region, cluster_name, service_name, table_name, bucket_name, execution_role_arn):
    ...
    alb_sg_id = ensure_security_group(ec2, ALB_SG_NAME, "Ingress publico HTTP para ALB do worker")
    task_sg_id = ensure_security_group(ec2, TASK_SG_NAME, "Ingress HTTP do ALB para tasks ECS")
    ...
    tg_arn = get_or_create_target_group(elbv2)
    lb_arn = get_or_create_load_balancer(elbv2, alb_sg_id)
    ...
    if status == "ACTIVE":
        ecs.update_service(service=service_name, forceNewDeployment=True, **service_kwargs)
    else:
